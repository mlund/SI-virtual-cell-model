...
protein_CI = file.topology.select('protein or name Cl') # Select protein and c.ions for reporters

### Add force to counterions ###
Cions = file.topology.select('name Cl')
force = CustomExternalForce('100*max(0, r-2.6)^2; r=sqrt((x-4.0)^2+(y-4.0)^2+(z-4.0)^2)')
